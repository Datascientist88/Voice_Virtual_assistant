...
const useVoiceAssistantAvatar = ({audioFileUrl}:UseVoiceAssistantAvatarProps) => {
  ...
  const animateOrb = () => {
    ...
    for (var e = 0; e < layersCount; e++) {
        ...
        for (var d = 0; d < layersCount; d++) {
            for (var a = 0; a < MAX; a++) {
              const b = p2[d * MAX + a];
              const b2 = p2[(d * (MAX + 1) + a) % MAX];
              const hueColor = (a / MAX) * 360;
              ctx.beginPath();
              ctx.strokeStyle = "hsla(" + hueColor + ", 70%, 50%, 0.15)";
              ctx.lineWidth = Math.pow(10, b[2]);
              ctx.moveTo(b[0] * s + SHIFT_VALUE, b[1] * s + SHIFT_VALUE);
              ctx.lineTo(b2[0] * s + SHIFT_VALUE, b2[1] * s + SHIFT_VALUE);
              ctx.stroke();
            }
          }
      }
