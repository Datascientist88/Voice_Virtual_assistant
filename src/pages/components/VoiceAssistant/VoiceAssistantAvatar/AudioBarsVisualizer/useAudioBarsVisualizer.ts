...
const SMOOTHING_TIME_CONSTANT = 0.85; // The bigger the number, the smoother it gets to draw the bars' height trace.
...
const useAudioBarsVisualizer = ({
  ...
  const init = async () => {
    ...
    if (!audioElement) {
      console.log("audioElment not found");
      return;
    }
    ...
    function visualize() {
      ...
      const drawAlt = function () {
        if (!source) {
          console.log("source undefined when drawing :(");
          return;
        }
